...
export default async function page({ params }: { params: { categoryDetails: string[] } }) {
    ...
    for(let i=0;i<products.length;i++){
         if (products[i].category._id==categoryDetails[0]) {
  ...
  return (
    <div className='mx-4'>
      <h1 className="text-3xl font-bold my-10 capitalize">{categoryDetails[1]}</h1>
      ...
      <div key={product._id} className='border-2 rounded-[10px] pb-7 px-5'>
        <Link href={"/products/"+product._id}>
          ...
          <span className='text-gray-700 capitalize object-cover text-[14px]'>{categoryDetails[1]}</span>
  ...
  <CardFooter className='p-0'>
      ...
      :<h2 className='text-center top-1/2 -translate-y-1/2 font-bold text-3xl'>No Products For This category</h2>}
    </div>
  )
}